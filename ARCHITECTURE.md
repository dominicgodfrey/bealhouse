...
| 3 | Rendering | Vite React SPA embedded in ONE Go binary via `embed.FS`; Go injects per-route meta + JSON-LD with live DB data. **Built** (`internal/httpx/meta.go`): the SPA fallback writes the page's own title, description, canonical, Open Graph tags and structured data into `<head>` before serving it, from the same read models the page's own API calls return — so the document a crawler indexes and the one a visitor reads cannot quote different rooms at different prices. Vite's static `<title>` is stripped, or every page would carry two. A description is the owner's own words or **absent**, never invented, on the same terms as the pages themselves; absolute URLs appear only with a `SITE_URL` to build them on. The booking flow and the console are `noindex` and carry no canonical. `robots.txt` and a `sitemap.xml` generated from live rooms sit beside it, ahead of the SPA fallback for the reason `/media/*` is. **Extended 2026-09-11:** the server also writes the page's *body* into `<div id="root">` (`internal/httpx/prerender.go`) from the rows the head was just built from, because most AI crawlers fetch HTML and never run the bundle. React clears it on its first render, so a visitor sees the page's words before the JavaScript arrives rather than a blank screen. The house is one schema.org entity with a stable `@id` that every other block references, the rooms carry a `BreadcrumbList`, the sitemap carries a real `<lastmod>` and no `<priority>`, `robots.txt` states `Allow: /` in one group, and `/llms.txt` is the same facts in plain text. The `<title>` is the constant "Beal House" on every page by the owner's instruction; what varies per page is `og:title` |
